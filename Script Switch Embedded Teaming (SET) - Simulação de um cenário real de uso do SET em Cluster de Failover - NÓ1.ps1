## Script Switch Embedded Teaming (SET) - Simulação de um cenário real de uso do SET em Cluster de Failover - NÓ1 - Créditos Gabriel Luiz - www.gabrielluiz.com ##
...
New-NetIPAddress -InterfaceAlias "vEthernet (Gerenciamento-Cluster)" -IPAddress 10.61.55.61 -PrefixLength 24 -Type Unicast | Out-Null

New-NetIPAddress -InterfaceAlias "vEthernet (Tráfego da VM)" -IPAddress 10.101.0.61 -PrefixLength 24 -Type Unicast -DefaultGateway 10.101.0.200 | Out-Null

Set-DnsClientServerAddress -InterfaceAlias "vEthernet (Tráfego da VM)" -ServerAddresses 10.101.0.100, 10.101.1.100 | Out-Null

New-NetIPAddress -InterfaceAlias "vEthernet (Migração ao vivo)" -IPAddress 10.60.55.61 -PrefixLength 24 -Type Unicast | Out-Null
